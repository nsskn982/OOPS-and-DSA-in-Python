...
\cite{cormen2022,sedgewick2011,knuth1997v1,goodrich2013,skiena2020},
and is enriched with findings from landmark research papers
\cite{bloom1970,pugh1990,dijkstra1959,kruskal1956,tarjan1975}.
Python-language specifics draw on \cite{vanrossum2009,lutz2013,ramalho2022}.
...
$\alpha = n/m$.  Python's \texttt{dict} resizes when $\alpha > 2/3$~\cite{knuth1998v2,goodrich2013}.
\end{definition}
...
For a more efficient mergeable priority queue, Fibonacci heaps achieve
amortised $O(1)$ decrease-key and $O(\log n)$ extract-min~\cite{fredman1987}.
...
stores partial sums in an implicit binary tree~\cite{knuth1998v3,cormen2022}.
...
Depth-First Search (DFS) was formalised by Tarjan~\cite{tarjan1972}, who used it
to compute strongly connected components in linear time.
Breadth-First Search (BFS) underpins shortest-path algorithms on unweighted graphs~\cite{cormen2022}.
...
Bellman-Ford handles graphs with negative edge weights and detects
negative-weight cycles~\cite{bellman1958,cormen2022}.
Its time complexity is $O(VE)$.

\begin{algorithm}
\caption{Bellman-Ford Shortest Path~\cite{bellman1958}}
\begin{algorithmic}[1]
\Procedure{BellmanFord}{$G, w, s$}
  ...
  \For{$i = 1$ \textbf{to} $|V|-1$}
    ...
    \EndFor
  \EndFor
  \For{each edge $(u,v) \in E$}  \Comment{negative-cycle check}
    \If{$d[u] + w(u,v) < d[v]$}
      \Return \textsc{false}  \Comment{negative-weight cycle exists}
    \EndIf
  \EndFor
  \Return \textsc{true}
\EndProcedure
...
using dynamic programming~\cite{floyd1962,cormen2022}.

\begin{algorithm}
\caption{Floyd-Warshall All-Pairs Shortest Paths~\cite{floyd1962}}
\begin{algorithmic}[1]
\Procedure{FloydWarshall}{$W$}  \Comment{$W$ is the $n\times n$ weight matrix}
  \State $D \leftarrow W$
  \For{$k = 1$ \textbf{to} $n$}
    \For{$i = 1$ \textbf{to} $n$}
      \For{$j = 1$ \textbf{to} $n$}
        \State $D[i][j] \leftarrow \min\!\bigl(D[i][j],\; D[i][k] + D[k][j]\bigr)$
      \EndFor
    \EndFor
  \EndFor
  \Return $D$
...
A \emph{persistent} data structure preserves all previous versions after
each update~\cite{okasaki1998}.  Path copying and fat-node techniques achieve
$O(\log n)$ update with full persistence.  Okasaki~\cite{okasaki1998} shows
that purely functional languages naturally yield persistent structures.

\subsection{Suffix Array and LCP Array}

A suffix array for string $S[0..n-1]$ is the sorted array of all suffix
start indices.  Kasai et al.~\cite{kasai2001} compute the LCP (Longest
Common Prefix) array in $O(n)$ time from a suffix array, enabling
$O(n\log n)$ construction of the full suffix data structure.
...
Kruskal's algorithm~\cite{kruskal1956,cormen2022} greedily adds the cheapest
edge that does not create a cycle, using a Disjoint Set Union structure~\cite{tarjan1975}
to detect cycles in $O(E\log E)$ time.

\subsection{String Matching Algorithms}

\paragraph{Knuth-Morris-Pratt (KMP).}
KMP~\cite{knuth1977kmp} preprocesses the pattern to build a failure function,
achieving $O(n+m)$ exact-match search (pattern length $m$, text length $n$).

\paragraph{Rabin-Karp.}
Rabin-Karp~\cite{karp1987} uses rolling polynomial hashing to match the
pattern in $O(n+m)$ expected time, making it suitable for multi-pattern search.

\paragraph{Aho-Corasick Automaton.}
The Aho-Corasick algorithm~\cite{aho1975} extends KMP to simultaneously search
for a set of $k$ patterns in $O(n + \sum |p_i| + z)$ time, where $z$ is the
total number of matches.

\begin{table}[h]
\centering
\caption{String search algorithm complexities.}
\begin{tabular}{llll}
\toprule
\textbf{Algorithm} & \textbf{Preprocessing} & \textbf{Search} & \textbf{Reference} \\
\midrule
Naive       & $O(1)$     & $O(nm)$      & --- \\
KMP         & $O(m)$     & $O(n)$       & \cite{knuth1977kmp} \\
Rabin-Karp  & $O(m)$     & $O(n+m)$ avg & \cite{karp1987} \\
Aho-Corasick& $O(\sum|p_i|)$ & $O(n+z)$ & \cite{aho1975} \\
\bottomrule
\end{tabular}
\end{table}
...
Consistent hashing was popularised by Amazon's Dynamo~\cite{decandia2007dynamo}
to distribute keys across a dynamic set of nodes with minimal remapping when
...
\begin{table}[h]
\centering
\caption{Python built-in data structures and their underlying implementations~\cite{vanrossum2009,lutz2013,ramalho2022}.}
\begin{tabular}{lll}
\toprule
